			...
			<div class="top-cart border-right-0">
				<div class="list-procart">
					...
					for ($i = 0; $i < $max; $i++) {
						...
						if ($color > 0) {
							$colorInfo = $cart->getColorInfo($color);
							if ($colorInfo['type_show'] == 1) {
								$pictureItems = $colorInfo['photo'];
								$linkPictureItems = UPLOAD_COLOR_L;
							} else {
							...
							<div class="form-row">
								<div class="pic-procart col-3 col-md-2">
									<a class="text-decoration-none" href="<?= $proinfo[$sluglang] ?>" target="_blank" title="<?= $proinfo['name' . $lang] ?>">
										<img onerror="this.src='<?= THUMBS ?>/85x85x1/assets/images/noimage.png';" src="<?= THUMBS ?>/85x85x2/<?= $linkPictureItems . $pictureItems ?>" alt="<?= $proinfo['name' . $lang] ?>" title="<?= $proinfo['name' . $lang] ?>" />
									</a>
