				...
				<div class="row">
					...
					<?php if(isset($config['setting']['worktime']) && $config['setting']['worktime'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							...
							<input type="text" class="form-control text-sm" name="data[options][worktime]" id="worktime" placeholder="Giờ làm việc" value="<?=(!empty($flash->has('worktime'))) ? $flash->get('worktime') : @$options['worktime']?>">
						</div>
					<?php } ?>
				</div>
							...
							<?php foreach($config['website']['lang'] as $k => $v) { ?>
								<div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
									...
									<?php if(isset($config['setting']['address']) && $config['setting']['address'] == true) { ?>
										<div class="form-group">
											<label for="address<?=$k?>">Địa chỉ (<?=$k?>):</label>
											<input type="text" class="form-control text-sm" name="data[address<?=$k?>]" id="address<?=$k?>" placeholder="Địa chỉ (<?=$k?>)" value="<?=(!empty($flash->has('address'.$k))) ? $flash->get('address'.$k) : @$item['address'.$k]?>" required>
										</div>
									<?php } ?>
								</div>
                            <?php } ?>
