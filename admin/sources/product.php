<?php
	...
	function deleteMan()
	{
		...
		if($id)
		{
			...
			if(!empty($row))
			{
				...
				/* Xóa color */
				$rowColor = $d->rawQuery("select id, photo from #_color where id_product = ?",array($id));
				...
				if(count($rowColor))
				{
					foreach($rowColor as $v)
					{
						$func->deleteFile(UPLOAD_COLOR.$v['photo']);
					}

					$d->rawQuery("delete from #_color where id_product = ?",array($id));
				}
	...
	function saveColor()
	{
		...
		if(empty($_POST))
		{
			$func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_color&type=".$type."&id_product=".$_REQUEST['id_product']."&p=".$curPage, false);
		}
		...
		if(!empty($response))
		{
			...
			if($id)
			{
				$func->redirect("index.php?com=product&act=edit_color&type=".$type."&id_product=".$_REQUEST['id_product']."&p=".$curPage."&id=".$id);
			}
			else
			{

				$func->redirect("index.php?com=product&act=add_color&type=".$type."&id_product=".$_REQUEST['id_product']."&p=".$curPage);
			}
		}
		...
		if($id)
		{
			...
			if($d->update('color',$data))
			{
				...
				$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_color&type=".$type."&id_product=".$_REQUEST['id_product']."&p=".$curPage);
			}
			else
			{
				$func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_color&type=".$type."&id_product=".$_REQUEST['id_product']."&p=".$curPage, false);
			}
		}
		else
		{
			$data['id_product'] = (isset($_REQUEST['id_product']) && $_REQUEST['id_product'] != '') ? $_REQUEST['id_product'] : 0;
			...
			if($d->insert('color',$data))
			{
				...
				$func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_color&type=".$type."&id_product=".$_REQUEST['id_product']."&p=".$curPage);
			}
			else
			{
				$func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_color&type=".$type."&id_product=".$_REQUEST['id_product']."&p=".$curPage, false);
			}
		}
	}
